...
const Navbar = () => {
  ...
  return (
    ...
    >
      <div className={`
        ${isScrolled ? `${navbarClass} shadow-lg ${isDarkTheme ? 'shadow-black/5' : 'shadow-black/5'}` : 'bg-transparent'} 
      ...
      `}>
        ...
        <nav className="hidden md:flex items-center gap-14">
          {[
            ...
            <a
              ...
              className={`text-sm font-medium relative px-3 py-1.5 overflow-hidden group ${
                activeLink === item.id 
                  ? 'text-foreground' 
            ...
            >
              ...
              <span className={`absolute bottom-0 left-0 w-full h-0.5 rounded-full transform origin-left scale-x-0 transition-transform duration-300 ${
                activeLink === item.id 
        ...
        <div className="flex items-center gap-3 md:hidden">
          ...
          >
            <div className="relative overflow-hidden w-7 h-7 flex items-center justify-center">
              <Menu size={20} className={`absolute inset-0 transition-all duration-500 ${
                isMenuOpen ? 'opacity-0 rotate-90 scale-0' : 'opacity-100 rotate-0 scale-100'
              } ${isDarkTheme ? 'text-white' : 'text-black'}`} />
              <X size={20} className={`absolute inset-0 transition-all duration-500 ${
        ...
        {/* Mobile menu */}
        <div className={`fixed inset-0 ${isDarkTheme ? 'bg-gradient-to-b from-black/95 to-gray-900/95' : 'bg-gradient-to-b from-white/95 to-gray-50/95'} backdrop-blur-md z-40 flex flex-col items-center justify-center transition-all duration-500 ${
